...
const HomeScreen = () => {
  ...
  return (
    <View style={{flex: 1, backgroundColor: Globals.PageColor, justifyContent: 'center', alignItems: 'center'}}>
      <AppButton onPress={() => ctx.setScreens([FormBuildingScreen])} outerStyle={{margin: 5, padding:5}} style={{width: 200, borderRadius: 15, height: 70}}>
        <Text selectable={false} style={{textAlign: 'center',  fontSize: 20, fontWeight: 'bold', color: 'white'}}>To Form Builder</Text>
      </AppButton>
      <AppButton onPress={() => ctx.setScreens([TeamsDataScreen])} outerStyle={{margin: 5}} style={{width: 200, height: 70}}>
        <Text selectable={false} style={{textAlign: 'center', fontSize: 20, fontWeight: 'bold', color: 'white'}}>To Teams Data Screen</Text>
      </AppButton>
      <AppButton onPress={() => ctx.setScreens([ComponenetTestingScreen])} outerStyle={{margin: 5}} style={{width: 200, height: 70}}>
